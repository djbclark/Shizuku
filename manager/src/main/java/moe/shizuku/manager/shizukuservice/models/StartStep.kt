...
sealed class StartStep(
    ...
    class EnableUsbDebugging(action: suspend () -> Unit) : StartStep(
        R.string.start_step_enabling_debugging, R.drawable.ic_outline_notifications_active_24, action
    )

    class EnableWirelessDebugging(action: suspend () -> Unit) : StartStep(
        R.string.start_step_enabling_wireless_debugging, R.drawable.ic_outline_notifications_active_24, action
    )
    ...
    class AwaitingAuthorization(action: suspend () -> Unit) : StartStep(
        R.string.start_step_awaiting_authorization, R.drawable.ic_outline_notifications_active_24, action
    )
